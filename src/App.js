...
function Clump({ mat = new THREE.Matrix4(), vec = new THREE.Vector3(), ...props }) {
  ...
  const [ref, api] = useSphere(() => ({ args: [1], mass: 1, angularDamping: 0.1, linearDamping: 0.65, position: [rfs(2), rfs(2), rfs(2)] }))
  useFrame((state) => {
    for (let i = 0; i < 1; i++) {
      ...
      api.at(i).applyForce(vec.setFromMatrixPosition(mat).normalize().multiplyScalar(-50).toArray(), [3, 0, 0])
    }
  })
